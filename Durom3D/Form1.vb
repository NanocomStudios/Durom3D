Public Class Form1
    ...
    Private Sub Form1_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        If My.Computer.FileSystem.FileExists("ffmpeg.exe") = False Then
            MsgBox("Error: 'ffmpeg.exe' file not found.", MsgBoxStyle.Critical, "Error")
            Me.Close()
        Else
    ...
    Private Sub Button5_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button5.Click
        bitmap = My.Computer.FileSystem.ReadAllBytes("1920x1080.bmp")
        ...
        For i = 0 To pointCount - 1
            ...
            point.X = getLoc(point.X, TrackBar1.Value + pointsZ(i), 1080) + ((1920 - 1080) / 2)
            point.Y = getLoc(point.Y, TrackBar1.Value + pointsZ(i), 1080)
        ...
        For i = 0 To point1.Length - 1
            drawLine(dots(point1(i)).X, dots(point1(i)).Y, dots(point2(i)).X, dots(point2(i)).Y, 1920, 1080)
        Next
    ...
    Sub drawLine(ByVal x1 As Integer, ByVal y1 As Integer, ByVal x2 As Integer, ByVal y2 As Integer, ByVal Rwidth As Integer, ByVal Rheight As Integer)
        Dim x As Integer
        Dim y As Integer
        ...
        If x1 > x2 Then
            For x = x2 To x1
                If x1 <> x2 Then
                    ...
                    If (x >= 0) And (x < Rwidth) And (y >= 0) And (y < Rheight) Then
                        bitmap(bitmap.Length - Rwidth + 1 + x - (y * Rwidth)) = 0
                    End If
                End If
            Next
        Else
        ...
        Else
            For y = y1 To y2
                If y1 <> y2 Then
                    x = (((y - y1) / (y1 - y2)) * (x1 - x2)) + x1
                    If (x >= 0) And (x < Rwidth) And (y >= 0) And (y < Rwidth) Then
                        bitmap(bitmap.Length - Rwidth + 1 + x - (y * Rwidth)) = 0
                    End If
                End If
            Next
    ...
    Sub renderVideo(ByVal outloc As String, ByVal asp As Byte, ByVal res As Byte)
        MsgBox(res & " " & asp)
        Dim Rheight As Single
        Dim Rwidth As Single
        Dim OutWidth As Single
        Dim OutRes As String = "1280x720"
        Select Case res
            Case 0
                Rheight = 720
                OutWidth = 1280
        ...
        For filecount = 0 To 360

            Select Case res
                Case 0
                    bitmap = My.Computer.FileSystem.ReadAllBytes("1280x720.bmp")
                Case 1
                    bitmap = My.Computer.FileSystem.ReadAllBytes("1920x1080.bmp")
            End Select


            RenderDialog.ProgressBar1.Value = (filecount / 360) * 100
            Dim s As Single = 1
            If RadioButton1.Checked Then
                rotate(s, "x")
            ElseIf RadioButton2.Checked Then
            ...
            For i = 0 To pointCount - 1
                ...
                point.X = getLoc(point.X, TrackBar1.Value + pointsZ(i), Rwidth) + ((OutWidth - Rwidth) / 2)
                point.Y = getLoc(point.Y, TrackBar1.Value + pointsZ(i), Rheight)
            ...
            For i = 0 To point1.Length - 1
                drawLine(dots(point1(i)).X, dots(point1(i)).Y, dots(point2(i)).X, dots(point2(i)).Y, OutWidth, Rheight)
            Next

            If filecount < 10 Then
                My.Computer.FileSystem.WriteAllBytes("render/0000" & filecount & ".bmp", bitmap, False)
            ElseIf filecount < 100 Then
                My.Computer.FileSystem.WriteAllBytes("render/000" & filecount & ".bmp", bitmap, False)
            Else
                My.Computer.FileSystem.WriteAllBytes("render/00" & filecount & ".bmp", bitmap, False)
            End If


        Next
        If My.Computer.FileSystem.FileExists(outloc) Then
            My.Computer.FileSystem.DeleteFile(outloc)
        End If
        Shell("ffmpeg -r 50 -s " & OutRes & " -i render/%05d.bmp -vcodec libx264 -crf 1  -pix_fmt yuv420p " & Chr(34) & outloc & Chr(34))
